...
description: "Case study: auditing and stabilizing Holloway Connect, a live construction operations platform, with test coverage, security fixes, and data recovery."
chatSummary: "Jack is the solo contract engineer on Holloway Connect, a live production operations platform for a construction contractor, where he ran a phased audit-and-stabilization program non-destructively against real production data. He grew the test suite from 0 to roughly 1,400 passing checks, pinning all money and payroll math to the penny before refactoring it into shared modules without changing a single computed value. He closed a critical cross-tenant data leak and added row-level security across all 47 entities, re-scoping the customer portal server-side so a test client dropped from seeing 223 jobs to exactly its own 1. He traced an over-broad auto-archive regression and recovered 91 wrongly-archived production jobs, restoring the active job list from 28 to 119. He built an idempotent geofenced payroll time-clock that reconciles double-taps, offline retries, and dual check-ins to exactly one entry, and consolidated the data-access layer into a query-factory that killed silent 5,000-record fetch truncations and a React Query cache-key collision."
highlights:
  [
