...
- **D-26:** **Phase 8 ships only when ALL FOUR pass**: (1) `npm run build` succeeds with zero errors, (2) `npm run lint` (eslint) clean, (3) `npm run check` (astro check — handles `.astro` types that bare `tsc --noEmit` misses; supersedes the D-26 v1 wording), (4) **manual chat smoke test** — start `npm run dev`, open the floating chat bubble on at least one stub page, send a message that triggers the Anthropic API (e.g., "show me a TypeScript snippet"), confirm SSE streaming response renders with Geist Mono in any `<code>` blocks, confirm focus trap works on Tab, confirm the bubble closes cleanly. **Smoke test is single-page only** (see D-29). The chat smoke test is the regression fingerprint that proves Phase 8 didn't break Phase 7 — every Phase 7 carry-over decision in STATE.md hinges on chat surviving. Plus: `npm run test` (vitest) — all existing tests must remain green.

### Late-binding decisions (added 2026-04-07 after research surfaced gaps)

- **D-27: chat.ts GSAP removal lands in the same commit as `npm uninstall gsap`.** Research found 6 GSAP call sites in `src/scripts/chat.ts` not enumerated in D-19/D-20: `animatePanelOpen`, `animatePanelClose`, `animateMessageAppear`, `startPulse`, `startTypingDots`, plus a `gsap.core.Tween` type annotation and a `pulseAnimation` module-level variable. D-14's "kill motion" scope is hereby extended: chat loses its GSAP-driven motion in Phase 8 (bubble pulse, panel scale-in, typing-dots bounce). All five animation helpers are replaced with no-ops. **Chat motion gap is a known, visible regression scoped to Phase 10 (CHAT-02 restyle) for restoration via CSS @keyframes if desired.** D-14 dominates D-19 on this specific collision. The `npm uninstall gsap` step CANNOT land separately from the chat.ts edit — they MUST be the same commit, or the build is red.

- **D-28: Chat char-count warning tokens both map to `--accent`.** D-19's mapping table is hereby extended:
  - `--token-destructive` (chat.ts:591-599, 500-char limit color) → `--accent`
  - `--token-warning` (chat.ts:591-599, 450-char limit color) → `--accent`
  Rationale: in this palette, accent red (#E63946) is the "signal" color; both warning tiers reinforce the urgency without adding tokens outside the 6-color promise. Phase 9 may differentiate later if needed; not Phase 8's call. With this, D-19 treats `--token-destructive` and `--token-warning` as **mapped, not deleted**.

- **D-29: Cross-page chat persistence is a known regression.** Removing `<ClientRouter />` (D-14) makes `transition:persist` a no-op. REQUIREMENTS.md CHAT-01 ("conversation persistence across page navigation") cannot be honored in Phase 8. Phase 8 explicitly descopes cross-page persistence — every page navigation is a full reload, and the chat panel resets. The `transition:persist` directive in `ChatWidget.astro` is removed in this phase along with the directive's runtime support. **Phase 8 smoke test (D-26) is single-page only** — multi-page navigation tests are NOT part of the gate. Phase 10 (CHAT-02 restyle) will restore cross-page persistence via localStorage-backed message restore. **Action item for Phase 10 planning:** REQUIREMENTS.md CHAT-01 needs reconciliation — either rewrite to match the new persistence model or add a CHAT-01.1 sub-requirement for the localStorage restore.

- **D-30: MobileMenu.astro gets a `DOMContentLoaded` fallback for `astro:page-load` in this phase.** Research found that `MobileMenu.astro:226` listens to `astro:page-load` (a `<ClientRouter />`-only event) with no fallback. Removing ClientRouter (D-14) silently breaks mobile nav initialization. To honor D-16 ("MobileMenu.astro is left fully wired and functional"), Phase 8 adds a fallback pattern matching the one chat.ts already uses: `if (document.readyState !== "loading") { initMobileMenu(); } else { document.addEventListener("DOMContentLoaded", initMobileMenu); }`. This fallback MUST land in the same commit as the `<ClientRouter />` removal — otherwise the build is green but mobile users have a dead menu. This is the only edit MobileMenu.astro receives in Phase 8 beyond the `/resume` nav-link removal and inline token rename; Phase 9 still owns the keep-or-rewrite decision per D-16.
...
- **REQUIREMENTS.md reconciliation for CHAT-01 "conversation persistence across page navigation"** — surface to user when Phase 10 plans CHAT-02. Phase 8 (D-29) descopes cross-page persistence; Phase 10 must either rewrite CHAT-01 to match the localStorage restore model or add CHAT-01.1 for the localStorage restore work.
- **Chat motion restoration (CSS @keyframes pulse / panel scale-in / typing-dots)** — Phase 10 (CHAT-02) decides whether to bring chat motion back via pure CSS, and what shape it takes. Phase 8 (D-27) ships chat as motionless.
