...
- [ ] **UAT-GAP-02** — #chat-panel opens on `pnpm dev` AND `pnpm preview` AND production. ChatWidget.astro inline style attribute contains no `display:` declaration (verified by tests/build/no-inline-display-on-chat-panel.test.ts); chat-panel-display.test.ts fixture mirrors real ChatWidget.astro markup; DEPLOY-GATE.md operator confirmation cleared before `git push origin main`. *Plan 17-08 — pending; runs LAST as release-blocker deploy gate.*
...
| UAT-GAP-02 | Phase 17 (Wave 10 gap-closure) | Pending (Plan 17-08 — release-blocker deploy gate) |
| UAT-GAP-03 | Phase 17 (Wave 8 gap-closure) | Implemented (Plan 17-09 — 2026-05-11) |
| UAT-GAP-04 | Phase 17 (Wave 9 gap-closure) | Pending (Plan 17-10) |
...
*Last updated: 2026-05-11 — Plan 17-09 close-out: UAT-GAP-03 (COPY button feedback window) implemented (2 atomic commits: dcf597b CSS rule + 10-test chat-copy-button.test.ts replacement; b35ad94 chat.ts COPY_FEEDBACK_MS = 1500 const + M3 inline color write deletion + B-iter2 timeout literal swap). pnpm test = 409 PASS / 2 SKIP / 0 FAIL (was 404/0/2 baseline; +5 net new tests; first all-green AND >= 405 tests). DEBT-05 anti-imperative-write pattern extended from `display` to `color`. Phase 17 progress: 8/10 plans complete. UAT-GAP-02 + UAT-GAP-04 still pending (Plans 17-08 + 17-10 across Waves 9-10). Plan 17-07 close-out below for context.
