...
- [ ] **UAT-GAP-03** — COPY button visibly transitions to COPIED for ~1500ms. .chat-copy-btn.copy-success CSS rule wired (opacity:1 + color:var(--accent)); chat.ts uses single COPY_FEEDBACK_MS = 1500 constant for both timeout windows; inline color writes removed from chat.ts (M3 — class toggle is single source of truth). *Plan 17-09 — pending.*
...
| UAT-GAP-03 | Phase 17 (Wave 8 gap-closure) | Pending (Plan 17-09) |
...
*Last updated: 2026-05-11 — Plan 17-07 close-out: Phase 17 RE-OPENED for gap closure (Wave 7-10 serial chain). UAT-GAP-01 implemented (chat voice-split — about-chat.ts + 6 MDX chatSummary + broadened leak guard + system-prompt.ts hardening + 21 voice-split tests across 2 files; 4 atomic commits ad9fdad → 537a0e6 → 05bf93d → 2aa627d; pnpm test 404 PASS / 2 SKIP / 0 FAIL — first phase-end with 0 FAIL since Plan 17-01). UAT-GAP-02..04 pending across Plans 17-09 → 17-10 → 17-08 (M-iter2 wave correction; 17-08 LAST as release-blocker deploy gate). Original Plan 17-06 close-out below for context.
