...
- [ ] **EXP-01**: An `experience` content collection with a Zod-validated schema (role, company, dates, stack/skills, summary) sourced from `Experience/*.md` via a sync pipeline mirroring the established `scripts/sync-projects.mjs` pattern
- [ ] **EXP-02**: Visitor can reach a dedicated Experience page from the primary navigation
...
- [ ] **EXP-06**: Experience entries render in reverse-chronological order with role, company, dates, and tech/skills visible at a glance
