...
A polished multi-page personal portfolio website for Jack Cutrara built with Astro 6, Tailwind CSS v4, and Geist typography deployed as a single Cloudflare Worker. The site showcases seven real project case studies and a dedicated Experience surface (a full Holloway contract case study + a lightweight Balfour Beatty entry), technical ability, and professional polish — functioning as a living complement to his resume that positions him as a new-grad software engineer with shipped production experience, worth interviewing. Live at jackcutrara.com.

After v1.4 the site ships with a typed Experience content pipeline (Holloway scannable summary → deep-dive, Balfour lightweight entry), a résumé-reconciled project set with a featured tier (SeatWatch / Multi-Chain EVM / NFL Prediction), honest new-grad-with-production-experience positioning across Core Value / About / education / metadata, and a Home experience teaser for the 30-second recruiter scan — on top of the v1.3 foundation: a tasteful motion layer, a knowledge-grounded chat widget powered by Anthropic prompt caching (now grounded in the Experience content + project #7 in third person per the CHAT-06 voice split), privacy-respecting analytics measuring recruiter engagement, end-to-end conversation visibility (KV transcript persistence + hourly cron sweep + Resend email delivery to Jack's Gmail), and a locked editorial design system documented in `design-system/MASTER.md` + `design-system/MOTION.md`.
...
## Latest Milestone: v1.4 Professional Experience (Shipped 2026-07-14)

**Delivered:** Added Jack's real professional experience to the site and repositioned him from "student with side projects" to a new-grad engineer with shipped production work — raising recruiter credibility while giving engineers a deep technical read.

**Shipped:**
- New Experience page (route + experience-first nav) — Holloway contract engagement as a scannable summary → full deep-dive case study
- Balfour Beatty 2023 internship as a lightweight earlier-work-history entry (structurally route-excluded, no full case study)
- Home page experience teaser (`01 EXPERIENCE`) for the 30-second recruiter scan
...
- Multi-Chain EVM / Multi-Dex Crypto Trader project (#7) synced onto the site via the existing `sync-projects.mjs` pipeline
- Projects reconciled with the résumé — SeatWatch, Multi-Chain EVM, NFL Prediction featured in a top tier; the other 4 accessible below (nothing deleted)
- Chat knowledge base (`portfolio-context.json`) refreshed to ingest the new experience + project #7, honoring the CHAT-06 voice split (third person in chat)

**Next milestone:** TBD — run `/gsd-new-milestone` to scope. One ship-time gate remains: the production-edge Lighthouse check fires during `/gsd-ship` (D-11).

## Current State

**Shipped:** v1.4 Professional Experience (2026-07-14) — 5 phases (21-25), 22 plans, 19/19 requirements; milestone audit passed (6/6 integration flows, 5/5 Nyquist-compliant) and archived
**Awaiting:** next milestone — run `/gsd-new-milestone` to scope. One ship-time gate remains: production-edge Lighthouse fires at `/gsd-ship` (D-11)
...
**Source LOC:** ~7,805 (src/ — astro, ts, css, mdx, md; +~15% from v1.3 close)
...
**Chat:** Claude Haiku via Anthropic SDK with `cache_control: ephemeral`; `portfolio-context.json` regenerated at build time from MDX + About + Resume + the Experience content + third-person `chatSummary`/`about-chat.ts` variants per CHAT-06 voice-split contract (corpus now 7 projects incl. Multi-Chain EVM untruncated + a 2-entry reverse-chron experience array, education single-sourced from `education.ts`, est_tokens=48735); the four D-14 chat-surface files byte-identical phase-wide (D-26 regression battery + D-15 SSE anchor held, `verify-phase25-invariants.mjs` committed-drift proof); TEST-03 Anthropic prompt-cache integrity preserved; `pnpm test` 692 PASS / 2 SKIP; `pnpm exec astro check` 0/0/0 (140 files)
...
- ✓ Experience content pipeline + collection — typed Zod-validated `experience` collection fed from `Experience/*.md` via `sync-experience.mjs` (fenced-block extraction mirroring `sync-projects.mjs`, idempotent `--check` drift gate wired into CI); `sortExperienceEntries` establishes the reverse-chronological ordering + field contract the Experience surface renders against; two entries double as schema-optionality fixtures (Holloway present-role/no endDate, Balfour empty techStack); zero new deps — EXP-01, EXP-06 closed — v1.4 (Phase 21)
...
- ✓ Projects reconciliation + featured tier — Multi-Chain EVM (#7) synced as a full case study through `sync-projects.mjs`; exactly three projects featured (SeatWatch / Multi-Chain EVM / NFL Prediction) in a two-tier FEATURED/MORE WORK layout on `/projects` + Home with the remaining four accessible below, driven by a single `featured`/`order` data-model distinction; #7 held out of the chat corpus until Phase 25 (D-15); capstone gate green + both human sign-offs approved — PROJ-01..04 closed — v1.4 (Phase 23)

- ✓ Chat knowledge refresh + milestone verification — `portfolio-context.json` regenerated with the Experience content (Holloway + Balfour as a reverse-chron `experience` array) and project #7 ingested (D-15 exclusion lifted), education single-sourced from `education.ts`, third-person voice-split (CHAT-06) held with the `checkFirstPersonLeaks` guard green; chat `about` P2 cut to match the site copy-review decision; the four D-14 gated files + `dependencies` byte-identical phase-wide (QA-01/QA-02 via `verify-phase25-invariants.mjs`); capstone gate green (build 0, 692 pass/2 skip incl. D-26 battery, astro check 0/0/0, drift 0), live chat-accuracy human sign-off approved; phase goal verified 17/17 — CHAT-10, CHAT-11, QA-01, QA-02 closed — v1.4 (Phase 25)

### Active

<!-- v1.4 Professional Experience fully validated — all high-level scope closed; detailed REQ-IDs in REQUIREMENTS.md -->

(none — v1.4 scope complete and verified end-to-end; production deploy + edge Lighthouse gate pending at `/gsd-ship`)
...
**v1.4 Professional Experience (2026-07-08 → 2026-07-14, ~7 days):** 5 phases (21-25), 22 plans, 53 tasks, 204 commits since the `v1.3` tag, 214 files changed repo-wide (+22,805 / −520; src/ alone 28 files +1,124 / −88). Source LOC src/ climbed ~6,802 → ~7,805 (+~15%). 19/19 requirements satisfied per the milestone audit (`status: passed`). 5/5 phases verified, 6/6 cross-phase integration flows wired end-to-end, 5/5 Nyquist-compliant. All cross-cutting gates HELD — the four D-14 chat-surface files (`BaseLayout.astro`/`global.css`/`chat.ts`/`api/chat.ts`) and `package.json dependencies` byte-identical phase-wide (QA-01/QA-02, proven by `verify-phase25-invariants.mjs` against a committed baseline); `pnpm test` 692 pass / 2 skip; `pnpm exec astro check` 0/0/0 (140 files). Chat corpus regenerated (projects 6 → 7 incl. Multi-Chain EVM untruncated, est_tokens=48735). Zero new runtime dependencies. One ship-time gate deferred to `/gsd-ship`: production-edge Lighthouse (D-11).
...
- `CHAT_RATE_LIMITER` Cloudflare binding — **reclassified WON'T-FIX at v1.4 close (2026-07-14)**. Code path defensively skips when binding absent (byte-identical from Phase 7); Free-tier accepted at portfolio scale. Revisit trigger: Anthropic spend or chat volume crossing the threshold that justifies a Workers Paid plan (~$60/yr) — the code path is ready and needs only the binding. (`.planning/todos/completed/2026-04-23-configure-chat-rate-limiter-binding.md`)
...
- SC4 organic-traffic idempotency-in-the-wild — 7-day soft cap elapsed 2026-05-20; Layer 1 (`delivered:{sid}` cursor) + Layer 2 (Resend Idempotency-Key 24h window) structurally unit-locked at both layers. Treat as closed absent a duplicate-email signal (no duplicate observed).
...
- Frontmatter attribution gaps — Phase 17 plans 17-07/08/09/10 + Phase 19 plans 19-01..04 (v1.3), plus Phase 22 (only EXP-02 listed; EXP-03/04/05 absent) and Phase 23 (PROJ-01..04 absent) (v1.4) omit/under-populate the `requirements-completed:` field. Requirements still covered by each phase VERIFICATION.md + traceability table — documentation hygiene only.
...
- v1.4 Phase 22 WR-01 — copied scroll-depth sentinels fire `scroll_depth` analytics on `/experience/holloway`, which the tracker's own comment scopes to "project detail pages only". Analytics scope drift, not a functional break.
- v1.4 Phase 22 WR-02 — the ~360-char first-person Holloway `summary` is reused verbatim as the detail-page meta/OG/Twitter description; truncates awkwardly in search results (SEO polish).
- v1.4 Phase 23 WR-02 — tautological wordcount test (`expect(true).toBe(true)`, soft `console.warn`); intentional per D-16 (the 600–900-word floor is enforced by human sign-off, not automation).
- Carry-forward UI/observability backlog (in `.planning/todos/pending/`): mobile-menu breakpoint 380→768px (Jack-approved, needs a frontend-design pass) and chat cache-hit-rate observability (low; touches the D-26-gated chat surface).
...
| Reuse the projects sync pipeline for experience (v1.4 Phase 21) | Parallel `Experience/*.md → experience` collection + `sync-experience.mjs`, not a new mechanism | ✓ Good — idempotent `--check` drift gate, Zod-validated, CI-wired |
| `hasCaseStudy` filter drives detail-route generation (v1.4 Phase 22 D-01) | Holloway builds `/experience/holloway`; Balfour structurally excluded from any detail route | ✓ Good — EXP-05 lightweight entry enforced at the `getStaticPaths` layer, not by convention |
| Single `featured`/`order` data-model distinction drives ordering everywhere (v1.4 Phase 23 PROJ-04) | One field pair drives featured-then-rest ordering on both `/projects` and Home; schema already carried the fields | ✓ Good — both surfaces number via `order`, no array-index desync |
| `education.ts` single source of truth (v1.4 Phase 24 D-10) | DERIVED alumniOf/hasCredential fragments feed both the JSON-LD Person schema and the chat education object; VT rendered as a transfer sub-note, never a credential | ✓ Good — one edit updates every education surface |
| Honest new-grad register, no seniority cosplay (v1.4 Phase 24 POS-01/02) | Gate A banlist enforces junior/senior/"5+ years" absence across all five site copy surfaces | ✓ Good — closed the MDX-only voice-gate scope gap |
| Deliberate D-15 → D-04 chat-corpus lift (v1.4 Phases 23→25 CHAT-10) | #7 held out of the chat corpus through Phase 23, then the `build-chat-context.mjs` exclusion lifted in Phase 25's atomic migration | ✓ Good — corpus stayed at 6 until the migration, then 7 untruncated; CHAT-06 leak guard green |
| Per-phase invariant baselines hashing gated files + deps (v1.4 Phases 24/25) | Prove phase-wide byte-identity against committed drift, not just a working-tree diff | ✓ Good — QA-01/QA-02 proven committed-drift-safe via `verify-phase2N-invariants.mjs` |
...
---
*Last updated: 2026-07-14 — v1.4 Professional Experience milestone completed and archived. 5 phases (21-25), 22 plans, 53 tasks; 19/19 requirements satisfied; milestone audit `status: passed` (5/5 phases verified, 6/6 integration flows wired, 5/5 Nyquist-compliant). src/ LOC ~6,802 → ~7,805; zero new runtime deps; the four D-14 chat-surface files + `dependencies` byte-identical phase-wide. Archives: [v1.4-ROADMAP.md](milestones/v1.4-ROADMAP.md) · [v1.4-REQUIREMENTS.md](milestones/v1.4-REQUIREMENTS.md) · [v1.4-MILESTONE-AUDIT.md](milestones/v1.4-MILESTONE-AUDIT.md). Tagged v1.4. Next: `/gsd-new-milestone` to scope the next milestone; one ship-time gate (production-edge Lighthouse) fires at `/gsd-ship` per D-11.*
